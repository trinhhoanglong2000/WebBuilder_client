...
export default function loadImageWithText(editor, opt = {}) {
  ...
  editor.TraitManager.addType("imageWithText-button-label", {
    createInput({ trait }) {
      const initValue =
        trait.target.view.el.querySelector(".btn").innerHTML || "";
      const placeholder = trait.get("placeholder") || "";
      const el = document.createElement("div");
  ...
  editor.TraitManager.addType("imageWithText-advance-setting", {
    createInput({ trait }) {
      ...
      const isHideButton = parent.get("components").models[2].getStyle()[
        "display"
      ];
      ...
      $(el)
        ...
        .prop("checked", isHideBorderText === "initial");

      return el;
    },

    onEvent({ elInput, component, event }) {
      const value = elInput.querySelector(
        "input.imageWithText-fullwidth"
      ).checked;
      const textPart = component.get("components").models[0].get("components")
        .models[1];
      const parent = textPart.get("components").models[0];
      const button = parent.get("components").models[2];
      ...
      const isHideButton = elInput.querySelector(
        "input.imageWithText-hideButton"
      ).checked;
      if (isHideButton) {
